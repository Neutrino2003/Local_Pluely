...
async fn run_vad_capture(
    ...
    let stop_listener = app.listen("manual-stop-continuous", move |_| {
        stop_flag_listener.store(true, Ordering::Release);
    });
    ...
    // Flush remaining speech buffer if we were in the middle of a sentence when stopped
...
pub async fn stop_system_audio_capture(app: AppHandle) -> Result<(), String> {
    ...
    // Keep cleanup delay short for low toggle latency.
    ...
    *state
    ...
    // Small extra delay to let platform capture state settle.
