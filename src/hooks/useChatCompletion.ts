...
export const useChatCompletion = (
  ...
  const emitRemoteChatIo = (
    ...
    invoke("remote_control_push_chat_io", {
      ...
      conversationTitle: messages?.title || null,
      isFinal,
    }).catch(() => { });
  };
  ...
  const appendTranscription = useCallback(
    async (transcription: string) => {
      ...
      const conversationIdForAppend = conversationId || generateConversationId("chat");
      const timestamp = Date.now();
      ...
      try {
        ...
        console.error("Failed to save ambient transcription:", error);
      }
    },
  ...
  // Listen for remote control chat messages
  useEffect(() => {
    let unlisten: any;
    ...
    const setupListeners = async () => {
      try {
        unlisten = await listen("remote-control-send-chat-message", (event: any) => {
        ...
        console.error("Failed to setup remote chat message listener:", err);
      }

      try {
        unlistenAmbient = await listen("ambient-transcription", (event: any) => {
        ...
        console.error("Failed to setup ambient transcription listener:", err);
      }
    };

    setupListeners();

    const handleRemoteStartChat = (e: any) => {
      const { conversationId: targetId, text } = e.detail;
      if (text && targetId === conversationId) {
        submit(text);
      }
    };
