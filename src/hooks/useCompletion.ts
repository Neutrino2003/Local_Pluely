...
import { useWindowResize, useWindowFocus } from "./useWindow";
...
export const useCompletion = () => {
  ...
  useWindowFocus({
    onFocusLost: () => {
      // When the user clicks on a background app, the Tauri window loses focus.
      // We must close the popover (shrink the window) to avoid blocking interactions.
      if (!keepEngaged && !state.isLoading) {
        setState((prev) => ({
          ...prev,
          input: "",
          response: "",
      ...
      setMicOpen(false);
      // Even if keepEngaged is true, we should probably reset if no response exists
      // but if there's a response, keepEngaged implies reading it alongside another window.
    },
  });
  ...
  const emitRemoteChatIo = (
    direction: "input" | "output",
    text: string,
    isFinal = false
  ) => {
    invoke("remote_control_push_chat_io", {
      direction,
      text,
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      emitRemoteChatIo("input", input);

      // Generate unique request ID
      const requestId = generateRequestId();
      currentRequestIdRef.current = requestId;

      // Cancel any existing request atomically
  ...
  // Listen for remote mobile chat actions and mirror them into overlay chat.
  useEffect(() => {
    let unlistenOpenChat: any;
    let unlistenSendChat: any;
    let unlistenAmbient: any;

    const setupListeners = async () => {
      try {
        unlistenOpenChat = await listen(
          ...
          async (event: any) => {
            ...
            try {
              ...
              console.error(
                "Failed to load remote-selected conversation:",
                error
              );
            }
          }
          ...
          async (event: any) => {
            ...
            if (
              ...
              try {
                ...
                console.error(
                  "Failed to load conversation for remote send:",
                  error
                );
              }
              setTimeout(() => submit(text), 50);
        ...
        console.error("Failed to setup remote mobile chat listeners:", error);
      }
    };
  ...
  useEffect(() => {
    invoke("remote_control_sync_overlay_state", {
      sttEnabled: micOpen,
    }).catch(() => { });
  }, [micOpen]);
