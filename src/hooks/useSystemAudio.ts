...
export function useSystemAudio() {
  ...
  const [includeMicInSystemAudio, setIncludeMicInSystemAudio] = useState(false);
          ...
          try {
            ...
            try {
              ...
              if (transcription.trim()) {
                ...
                // Emit transcription for active chat windows to capture and process with AI
                emit("ambient-transcription", { text: transcription }).catch((err) =>
  ...
  const handleQuickActionClick = async (action: string) => {
    ...
    // Emit the quick action to be captured by the main chat UI that handles transcriptions
    emit("ambient-transcription", { text: action }).catch((err) =>
      console.error("Failed to emit quick action transcription", err)
    );
  };
  ...
  // Removed processWithAI. Background audio no longer processes its own isolated AI responses.
