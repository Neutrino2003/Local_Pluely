...
export const AudioSelection = () => {
  ...
  return (
    <div id="audio" className="space-y-1 flex flex-col gap-4">
      ...
      {/* Meeting Input Mix Section */}
      <div className="space-y-3">
        <Header
          title="Meeting Input Mix"
          description="Choose whether system audio capture should also include your microphone for speaker-separated transcripts."
        />

        <div className="rounded-md border border-input/50 p-4 space-y-3 bg-muted/20">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1">
              <Label className="text-sm font-medium">
                Include my microphone during system capture
              </Label>
              <p className="text-xs text-muted-foreground">
                {includeMicInSystemAudio
                  ? "Enabled: Pluely will process both meeting audio and your mic as separate sources."
                  : "Disabled: Pluely will only process system audio from meeting participants."}
              </p>
            </div>
            <Switch
              checked={includeMicInSystemAudio}
              onCheckedChange={setIncludeMicInSystemAudio}
            />
          </div>
          <p className="text-xs text-muted-foreground/80">
            Recommendation: Keep this off if you only want remote speakers.
            Turn it on when you want your own speech labeled separately.
          </p>
        </div>
      </div>
