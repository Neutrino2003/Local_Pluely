export const SPEECH_TO_TEXT_PROVIDERS = [
  {
    ...
    // NOTE: This provider is handled natively via the `transcribe_local_whisper` Tauri command.
    // The curl field below is a stub and is never used for HTTP requests.
    curl: `curl -X POST "http://127.0.0.1:8080/v1/audio/transcriptions" \\ 
      -F "file={{AUDIO}}" \\ 
      -F "model={{MODEL}}"`,
